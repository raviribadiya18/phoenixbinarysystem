<?php
Class Other_document_module extends CI_Model
{
 	
	
	function get_document()
 	{	
		
		$this -> db -> select('membermaster.fname,membermaster.lname');
		
		$this -> db -> select('sub_membermaster.fname as sub_fname,sub_membermaster.lname  as sub_lname');
		
   		$this -> db -> select('other_document_master.*');
		
   		$this -> db -> from('other_document_master');
		
		$this -> db -> JOIN('membermaster','membermaster.usercode=other_document_master.usercode','LEFT');
		
		$this -> db -> JOIN('sub_membermaster','sub_membermaster.id=other_document_master.sub_id','LEFT');
		
   		$this -> db -> where('other_document_master.status !=', 'Delete');
		
		$this -> db -> where('membermaster.status', 'Active');
		
		$this -> db -> where('sub_membermaster.status', 'Active');
		
		$this -> db -> order_by('other_document_master.id', 'Desc');
		
		$query = $this -> db -> get();
		
    	$the_content = $query->result_array();
		
    	return $the_content;
		
 	}

	  function get_record($eid){

			$this -> db -> select('other_document_master.*');

			$this -> db -> from('other_document_master');

			$this -> db -> where('other_document_master.status !=', 'Delete');

			$this -> db -> where('other_document_master.id', ''.$eid.'');
		  
			$query = $this -> db -> get();

			$the_content = $query->result_array();

			return $the_content;

		}
	
	
	function get_all_client_list()
 	{	
   		$this -> db -> select('membermaster.*');
		
   		$this -> db -> from('membermaster');
		
   		$this -> db -> where('membermaster.status', 'Active');
		
		$this -> db -> where('membermaster.role', 'client');
		
    	$query = $this -> db -> get();
		
    	$the_content = $query->result_array();
		
    	return $the_content;
		
 	}
	
}
?>

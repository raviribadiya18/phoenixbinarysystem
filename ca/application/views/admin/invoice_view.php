...
<script>
	...
	$(document).on('change','#all_option',function(e){
		//alert($(this).val());
		var url='<?=file_path('admin')?><?=$this->uri->rsegment(1)?>/invoice_view/<?=$this->uri->rsegment(3)?>/'+$(this).val()
		//alert(url);
    ...
    <div class="panel-heading">
      <h4 class="panel-title"><?=$sub_title?>
      	<span class="pull-right">
                    ...
                    <a class="btn btn-success btn-small" href="<?=file_path('admin')?><?=$this->uri->rsegment(1)?>/invoiceaddnew/add/<?=$this->uri->rsegment(3)?>">
            		Add New
            		</a></div>
    	 ...
    	 <div>
            <select name="all_option" id="all_option" class="form-control" style="width:200px;margin-bottom:15px;">
              ...
              <?php for($i=0;$i<count($all_option);$i++){
							$sel=($this->uri->rsegment(4)==$all_option[$i]['id'])? "selected='selected'" : "";

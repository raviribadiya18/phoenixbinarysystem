	...
	function __construct()
 	{
		...
		ob_end_flush();
   		...
   		$this->load->model('admin/Employee_module','ObjM',TRUE);
	...
	function listing(){
		...
		for($i=0;$i<count($result);$i++){
				...
				$html.='<tr>
						...
						<td>'.$result[$i]['city'].'</td>
						<td>'.date('d-m-Y',strtotime($result[$i]['join_date'])).'</td>
						...
						<ul class="dropdown-menu pull-right">
							<li><a href="'.file_path('admin').'change_account/member/'.$result[$i]['usercode'].'">Swap to Employee Account</a></li>
...
//             );
	...
	protected function _insert(){
		...
		if($_POST['mode']=='add')
		{
			...
			$data['username']		=	filter_data($_POST['fname']);
			...
			$u_data  =	array(
				'username'=>strtolower(filter_data($_POST['fname'])).$user_code
			);			

...
class Roc_master extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('admin/Roc_master_module', 'ObjM', TRUE);
	...
	public function view() {
		...
		$page_info['menu_id'] = 'menu-roc';

		$page_info['page_title'] = 'ROC Document';
	...
	function listing() {
		...
		for ($i = 0; $i < count($result); $i++) {
			...
			if ($result[$i]['upload_file'] != "") {
				...
				$file = "<a href='" . base_url() . "upload/web/roc/" . $result[$i]['upload_file'] . "' download>Download File Here</a>";
	...
	protected function _insert() {
		...
		if ($_POST['mode'] == 'add') {
			...
			$config['upload_path'] = './upload/web/roc/';
			...
			for ($i = 0; $i < $cpt; $i++) {
				...
				if ($files['upload_file']['name'][$i]) {
					...
					$fileName = 'roc_file_' . $rand . '' . $_FILES['upload_file']['name'];
					...
					if ($this->upload->do_upload()) {
						...
						$this->comman_fun->additem($data, 'roc_master');
		...
		if ($_POST['mode'] == 'edit') {
			...
			$this->comman_fun->update($data, 'roc_master', array('id' => $_POST['eid']));
	...
	function status_update($st, $eid) {
		...
		$record = $this->comman_fun->get_table_data('roc_master', array('id' => $eid));

		$data['status'] = $st;

		$this->comman_fun->update($data, 'roc_master', array('id' => $eid));
	...
	function delete_record($eid) {
		...
		$url = './upload/web/roc/' . $record[0]['upload_file'];
	...
	function handle_upload() {
		if (isset($_FILES['upload_file']) && !empty($_FILES['upload_file']['name'])) {
			...
			//$fileName							=	'roc_file_'.$_FILES['upload_file']['name'];
			$fileName = str_replace(" ", "", $fileName);
			$config['file_name'] = $fileName;
			$this->upload->initialize($config);
